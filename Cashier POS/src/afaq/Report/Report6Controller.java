 ...
 * @author AMR SAID
 */
public class Report6Controller implements Initializable, ControlledScreen {
    ...
    public void FillTable() throws SQLException {
        ...
        DB.rs = DB.statemen.executeQuery("SELECT bill_no,items,cus_name,qty,sell_price,tot_price,sell_date  FROM sell_items WHERE barcode = ''");

        while (DB.rs.next()) {

            data.add(new TableReport(DB.rs.getString("sell_date"), DB.rs.getString("bill_no"), DB.rs.getString("items"), DB.rs.getString("qty"), DB.rs.getString("tot_price"), DB.rs.getString("sell_price")));
    ...
    public void SearchParcode() throws SQLException {
        ...
        String Sql = "SELECT bill_no,items,cus_name,qty,sell_price,tot_price,sell_date  FROM sell_items WHERE barcode = '' and  sell_date between '" + date + "' and '" + date2 + "'";

        if (!comboBox1.getSelectionModel().isEmpty()) {
            Sql = "SELECT bill_no,items,cus_name,qty,sell_price,tot_price,sell_date  FROM sell_items WHERE barcode = '' and  items = '" + comboBox1.getValue() + "' and  sell_date between '" + date + "' and '" + date2 + "'";
    ...
    public void menucustomer() throws SQLException {
        ...
        while (DB.rs.next()) {
            Cus_Name.add(DB.rs.getString("s_name"));
