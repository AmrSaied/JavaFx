 ...
 * @author AMR SAID
 */
public class Report9Controller implements Initializable  , ControlledScreen {
    ...
    public void FillTable() throws SQLException {
        ...
        while (DB.rs.next()) {
            ...
            data.add(new TableReport(DB.rs.getString("ret_date"), DB.rs.getString("barcode"), DB.rs.getString("items"), DB.rs.getString("cus_name"), DB.rs.getString("qty"), DB.rs.getString("tot_price"), DB.rs.getString("sell_price")));
    ...
    public void SearchParcode() throws SQLException {
        ...
        String Sql = "SELECT * FROM  sell_ret WHERE ret_date between '" + date + "' and '" + date2 + "'";
        if (!TFbarcode.getText().equals("")) {
            Sql = "SELECT * FROM  sell_ret WHERE barcode = '" + TFbarcode.getText() + "' and  ret_date between '" + date + "' and '" + date2 + "'";
        }
  
        if (!comboBox1.getSelectionModel().isEmpty()) {
            Sql = "SELECT * FROM  sell_ret WHERE cus_name = '" + comboBox1.getValue() + "' and  ret_date between '" + date + "' and '" + date2 + "'";

        }
              if (!TFbarcode.getText().equals("") && !comboBox1.getSelectionModel().isEmpty()) {
            Sql = "SELECT * FROM  sell_ret WHERE barcode = '" + TFbarcode.getText() + "' and cus_name = '" + comboBox1.getValue() + "' and  ret_date between '" + date + "' and '" + date2 + "'";
