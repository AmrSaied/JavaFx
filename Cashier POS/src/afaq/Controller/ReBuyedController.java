 ...
 * @author AmrSaid
 */
public class ReBuyedController implements Initializable, ControlledScreen {
    ...
    public void SearchBarCode() throws SQLException {
        ...
        if (DB.rs.next()) {
            ...
            double buy_price = DB.rs.getDouble("buy_price");
            pricebuyed.setText(DB.rs.getString("buy_price"));

            double CastAmount = Double.parseDouble(amount.getText()) * buy_price;
    ...
    public void insertRebyed() throws SQLException {
        ...
        try {
            ...
            if (value1.equals("") && value2.equals("") && value3.equals("null") && value4.equals("") && value5.equals("") && value6.equals("") && value7.equals("")) {
                ...
                DB.statemen.executeUpdate("INSERT INTO buy_ret(barcode, items, com_name, qty, buy_price, tot_price, ret_date, user_name, action) VALUES ('" + value1 + "','" + value7 + "','" + value3 + "','" + value2 + "','" + value4 + "','" + value5 + "','" + value6 + "','" + value8 + "','مرتجع')");
                UpdataStock();
                fillTable();
                ClearAdd();
            }
    ...
    public void fillTable() throws SQLException {
        data.clear();
        DB.rs = DB.statemen.executeQuery("SELECT * FROM buy_ret");

        while (DB.rs.next()) {
            data.add(new TReButed(DB.rs.getString("id"), DB.rs.getString("barcode"), DB.rs.getString("items"), DB.rs.getString("com_name"), DB.rs.getString("qty"), DB.rs.getString("buy_price"), DB.rs.getString("tot_price"), DB.rs.getString("ret_date")));
    ...
    public void fillTable(String value1) throws SQLException {
        data.clear();
        DB.rs = DB.statemen.executeQuery("SELECT * FROM buy_ret WHERE ret_date =  '" + value1 + "'");
    ...
    public void Deleteitem() throws SQLException {
        ...
        try {
            ...
            String Sql = " DELETE FROM  buy_ret WHERE id = '" + InsertData.getId() + "'";
 ...
 TReButed InsertData = (TReButed) tablebuyed.getSelectionModel().getSelectedItem();
    ...
    public void Selecteditem() throws SQLException {
        ...
        TReButed InsertData = (TReButed) tablebuyed.getSelectionModel().getSelectedItem();
        String Sql = " SELECT * FROM buy_ret WHERE id =  '" + InsertData.getId() + "'";
