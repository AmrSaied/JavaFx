...
public class BillBuyedController implements Initializable, ControlledScreen {
    ...
    ObservableList<TFullCashier> data = FXCollections.observableArrayList();
    ...
    public void BillNo() throws SQLException {
        DB.rs = DB.statemen.executeQuery("SELECT * FROM buy_bills");
        DB.rs.last();
    ...
    public void SearchBarCode() throws SQLException {
        ...
        if (DB.rs.next()) {
            ...
            double Sell_Price = DB.rs.getDouble("buy_Price");
            pricebuyed.setText(DB.rs.getString("buy_Price"));
    ...
    public void AddtoTable() throws SQLException {
        try {
            ...
            DB.rs = DB.statemen.executeQuery("SELECT * FROM stock WHERE Barcode = '" + barcodenumber.getText() + "'");
            DB.rs.next();
            data.add(new TFullCashier(DB.rs.getString("ID"), barcodenumber.getText(), DB.rs.getString("items"), amount.getText(), DB.rs.getString("Buy_Price"), totalAmout.getText(), DB.rs.getString("Buy_Price")));
            ClearAdd();
            SearchBarCode();
            SumTotal();
        } catch (Exception EX) {
    ...
    public void SumTotal() {
        ...
        for (int i = 0; i < data.size(); i++) {
            totall = totall + Double.parseDouble(data.get(i).getTotal());
    ...
    public void InsertBill() throws SQLException {
        try {
            ...
            String Sql5 = "SELECT * FROM buy_bills Where bill_no ='" + SearchBillN.getText() + "'";
            DB.rs = DB.statemen.executeQuery(Sql5);
            if (DB.rs.next()) {
                String Sql = "delete FROM `buy_bills` WHERE `bill_no` = '" + BillNumber.getText() + "'";
                String Sql2 = "delete FROM `buy_items` WHERE `bill_no` = '" + BillNumber.getText() + "'";
            ...
            if (!Value2.equals("")) {
                ...
                if (rb2.isSelected()) {
                    ...
                    if (cahmged <= 0) {
                        ...
                        String Sql = "INSERT INTO `buy_bills` ( `bill_no`, `com_name`, `tot_price`, `paid`, `change`, `bill_date`, `user_name`) VALUES ('" + Value1 + "', '" + Value2 + "', '" + Value3 + "', '" + Value4 + "', '" + Value5 + "', '" + Value6 + "', '" + Value7 + "');";
                        System.out.println(Sql);
                        DB.statemen.executeUpdate(Sql);
                    
                        for (int i = 0; i < data.size(); i++) {
                            int y = i + 1;
                            DB.statemen.executeUpdate("INSERT INTO buy_items( bill_no, barcode, items, qty, buy_price, tot_price, buy_date, com_name, action) VALUES ('" + Value1 + "','" + data.get(i).getParcode() + "','" + data.get(i).getName() + "','" + data.get(i).getAmount() + "','" + data.get(i).getPricebuy() + "','" + data.get(i).getTotal() + "','" + Value6 + "','" + Value2 + "','شراء')");
                        }
                        UpdataStock();
                        ClearAll();
                    }
                    ...
                    String Sql = "INSERT INTO `buy_bills` ( `bill_no`, `com_name`, `tot_price`, `paid`, `change`, `bill_date`, `user_name`) VALUES ('" + Value1 + "', '" + Value2 + "', '" + Value3 + "', '" + Value3 + "', '0', '" + Value6 + "', '" + Value7 + "');";
                    DB.statemen.executeUpdate(Sql);
                    System.out.println(Sql);
                    for (int i = 0; i < data.size(); i++) {
                        int y = i + 1;
            ...
            Erromassage.setText("ادخل البيانات المدلوبة");
        }
    }

    @FXML
    public void CloseScene() {
        ScreensController.p4 = false;
    ...
    public void SearchBill() throws SQLException {
        try {
            ...
            String Sql2 = "SELECT * FROM buy_items Where bill_no = '" + SearchBillN.getText() + "'";
            System.out.println(Sql);
            data.clear();
            ...
            while (DB.rs.next()) {
                data.add(new TFullCashier(DB.rs.getString("id"), DB.rs.getString("barcode"), DB.rs.getString("items"), DB.rs.getString("qty"), DB.rs.getString("buy_price"), DB.rs.getString("tot_price"), DB.rs.getString("buy_price")));
...
//            String TimeN = "الوقت  " + time.getText();
            ...
            g2.drawString("فواتير الموردين", 130, 17);
...
//            g2.drawString(TimeN, 7, 46);
            ...
            for (int i = 0; i < data.size(); i++) {
                ...
                g2.drawString(data.get(i).getName(), 120, y);

                g2.drawString(data.get(i).getAmount(), 180, y);

                g2.drawString(data.get(i).getPricebuy(), 65, y);
    ...
    public void UpdataStock() throws SQLException {
        ...
        for (int i = 0; i < data.size(); i++) {
            String sql = "SELECT * FROM  `stock`  WHERE Barcode = " + data.get(i).getParcode() + "";
            DB.rs = DB.statemen.executeQuery(sql);
            DB.rs.next();
            int numberStock = DB.rs.getInt("QTY") + Integer.parseInt(data.get(i).getAmount());
            sql = "UPDATE stock SET QTY='" + numberStock + "' WHERE  Barcode = " + data.get(i).getParcode() + "";
            DB.statemen.executeUpdate(sql);
